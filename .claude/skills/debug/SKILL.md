...
| `<dir> is already open by process N` at boot | Working as intended. PGlite takes no lock of its own, so `src/db/data-dir-lock.ts` takes one — two openers of one `.pglite` fork the database silently and the last to close overwrites the other. Stop the pid it names, or give this process `PGLITE_DATA_DIR=<somewhere else>`. A lock left behind by a *killed* server is taken over automatically, so this never means "delete the lock file" |
