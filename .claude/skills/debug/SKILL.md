...
| `dev: restarting: held N MB … past the budget` | Working as intended, not a bug to chase. The request in flight was lost; the next compile is warm. `DIVEDAY_DEV_MEMORY_BUDGET_MB` moves the line, `0` turns it off |
...
| Stale/weird dev data | `pnpm db:reset` (wipes `.pglite/`; next boot re-migrates + re-seeds). It **refuses while a dev server is running** and names the pid, because PGlite takes no lock on its data directory: deleting it under a live server does not reach that server, which keeps answering from the handles it holds and discards every later write on exit. The same missing lock is why two processes must never share one `.pglite` — they fork it silently and the last to close overwrites the other |
