...
- **One surface inverts that order, on purpose**: the lobby departures board
  (`src/app/board/[token]/page.tsx`) prefers `shops.default_locale` outright and negotiates only
  when the shop's stored locale is one DiveDay no longer carries. A wall screen has no reader — no
  cookie a staffer could ever set on a television, and factory-default hardware announces
  `Accept-Language: en-US` in a Spanish-speaking lobby. It is also the one place `<html lang>` and
  the content's language may disagree, so that route's `<main>` carries its own `lang`. Adding a
  second such surface is a decision, not a refactor; do not "fix" this one back.
