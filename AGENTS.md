...
| Where staff can go (nav tabs, ⌘K "Go to", `g`-key shortcuts) | one registry, `src/lib/staff-destinations.ts` — path, permission gate, badge source, shortcut key, nav group. `src/components/ShopNavLinks.tsx` (header tabs), `src/components/StaffTabBar.tsx` (the phone dock), `src/components/ShopIdentityMenu.tsx` (Settings, behind the shop's name), `src/components/search/CommandPalette.tsx`, and the shortcut sheet all derive from it; `src/components/ShopNav.tsx` resolves the one label record they share. Add a destination there, never to a consumer. **Five `primary` tabs, six is the ceiling** — the dock has no squeezing room, so a sixth means demoting one, not growing the bar |
