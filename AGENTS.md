...
| A shop's own WhatsApp sender | `src/db/whatsapp-accounts.ts` + `src/app/shop/[shopSlug]/settings/whatsapp/`; tokens sealed by `src/lib/secret-box.ts`; setup in [docs/engineering/whatsapp-cloud-api-runbook.md](docs/engineering/whatsapp-cloud-api-runbook.md) |
