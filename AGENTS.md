...
| Course pages (public content / staff roster + editor) | diver-facing `src/app/s/[shopSlug]/courses/**`; staff roster and editor `src/app/shop/[shopSlug]/courses/**`; content shapes and parsers in `src/lib/courses.ts`; DiveDay-published templates in `src/db/course-templates.ts`. The roster reads in **progression order** — `courseProgressionRank` in `src/lib/courses.ts`, applied by `pagedCourses` — never alphabetically, and the shop-built certification paths it replaced are gone (ADR 20260805-remove-certification-paths) |
