...
- **Every delete is soft, and the word on screen is still "Delete."** A user pointing at a thing and
  ...
  **Never say so.** Not Archive, Unarchive, Deactivate, Retire, Hide, or "soft delete" in anything a
  ...
  vocabulary half over the message bundles. Two divergences are known and not yet fixed:
  `deleteTrip` (`src/db/trips-schedule.ts`) still hard-deletes an empty departure and five child
  tables (FU-20260820-soft-delete-a-departure), and the *internal* vocabulary is still the old one —
  `archiveCertification`, `waiver_templates.archived_at` (FU-20260820-internal-archive-vocabulary).
  Neither reaches a reader, which is why neither is a gate.
