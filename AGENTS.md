...
| AWS credentials, and what deploying still leaves for a human | one Secrets Manager secret holding a filled-in `.env.example` (§13 of `infra/lib/infra-stack.ts`), and one registry of manual steps (§14) that renders to both stack outputs and the generated [docs/engineering/manual-actions.md](docs/engineering/manual-actions.md). Add a step to the registry, never to a runbook; regenerate with `pnpm test infra -u`. Reasoning lives in [docs/engineering/infrastructure-runbook.md](docs/engineering/infrastructure-runbook.md) |
