...
- **Every delete is soft, and the word on screen is still "Delete."** A user pointing at a thing and
  ...
  **Never say so.** Not Archive, Unarchive, Deactivate, Retire, Hide, or "soft delete" in anything a
  ...
  public: an unfiltered read shows an anonymous visitor a departure the shop took off the board. One
  divergence remains, and it reaches no reader, which is why it is not a gate: the *internal*
  vocabulary is still the old one — `archiveCertification`, `waiver_templates.archived_at`
  (FU-20260820-internal-archive-vocabulary).
