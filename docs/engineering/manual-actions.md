...
[3] Get the account verified for CloudFront
    ...
    produces MediaDistribution (infra-stack.ts S11b) can be created. Until then the stack ships without it: cdk.json carries cloudfrontVerified: false, and every course photo, recap photo, dive-site image and shop logo answers 403 (issue #1013) -- the state the account is in either way. This account cleared the gate on 2026-09-03 and the committed value is true; the step stands for the next account, not this one.
...
[6] Mint a GitHub token for the CI deploy job's own gh CLI calls
    ...
    store    GitHub repo Settings -> Environments -> infra-deploy -> Environment secrets -> GH_TOKEN. On the environment rather than the repository, so it exists only for the one job the required-reviewer approval already gates.
...
[11] Request SES production access
    ...
    verify   aws sesv2 get-account --region us-east-1 --query ProductionAccessEnabled
...
[13] Confirm the observability alarm subscription email
    ...
    run      Open all three 'AWS Notification - Subscription Confirmation' mails sent to the alert address and click Confirm subscription in each.
             ...
             aws sns list-subscriptions-by-topic --region us-east-1 --topic-arn <SesAlarmTopicArn>
             aws sns list-subscriptions-by-topic --region us-east-1 --topic-arn <UptimeAlarmTopicArn>
