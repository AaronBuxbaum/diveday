...
Every prefix in `CAPABILITY_ROUTE_PREFIXES` (`src/lib/capability-urls.ts`)
carries a bearer capability in the path itself — anyone holding the URL has the
access it grants. Today that is `/waivers`, `/ready`, `/recap`, `/verify`,
`/reset-password`, `/invite`, `/confirm-contact`, `/claim`, `/calendar`,
`/unsubscribe`, `/board`, `/shelf`, `/gift` and `/check-in`. **That list is the
authority and this sentence is a copy of it**; read it there rather than
counting here, because a count in prose is the thing that goes stale — this
paragraph said "a ninth capability" while the list held fourteen.

Three more travel as *query parameters* instead of path segments, and
`CAPABILITY_QUERY_PARAMS` holds them: `?booking=` (the schedule-confirmation
`confirm` purpose, including in the URL Stripe checkout redirects back to),
`?handoff=` (the ten-minute booking handoff), and `?gate=`. `?gate=` is the odd
one — it is not a bearer credential at all, since its signature unlocks a
sentence rather than a resource and verification is bound to an id already in
the path. It is redacted for what its value *says*: a person id sitting beside
the certification level that person holds. So it needs no revoke row below, and
the next sweep should stop re-finding it as a hole.
...
redacts each of those query values on *any* path — before Vercel Analytics or
...
| Booking handoff (`/s/[shopSlug]/trips/[id]?handoff=[token]`) | Hashed, expiring, revocable **and single-use** row in `booking_capabilities` (`purpose = 'handoff'`; minted, read and consumed by `src/db/booking-handoff.ts`, [ADR 20260906-before-you-ask](../architecture/decisions/20260906-before-you-ask.md) decision 3) | Yes — call `revokeBookingCapabilities(db, { shopId, bookingId, purpose: "handoff" })`. But rotation is nearly moot here, because two bounds get there first. It lives **ten minutes** (`HANDOFF_TTL_MS`, `src/lib/booking-handoff.ts`), and it is **single-use**: `consumeBookingHandoff` stamps `revoked_at` on the token the booking page just read, idempotently, so a URL someone forwarded is usually already dead in the hand. A cancelled booking's or called-off trip's handoffs fail closed inside `verifyBookingCapability` like every other purpose, and the same per-purpose live cap applies. **What an exposed one buys is more than the folded facts** — `readKnownDiver` returns the lead diver's `fullName`, `email` and `phone`, and the booking form prefills them, so treat this as an identity disclosure and not only a convenience panel. The lead's own phone is deliberately in that payload (the form field is the door to change it); the emergency contact's never is. Nothing on `NEVER_HANDED_OFF` folds in at any depth — no medical answers, no date of birth, no emergency-contact phone (the contact's *name* only), no identifier, no party size or capacity — and `src/lib/booking-handoff.test.ts` walks the fold's output to hold that. Unlike `/ready` and `/claim`, **a dead handoff names nobody**: every refusal returns the same `null` and the page renders the cold form it ships, so there is no permanent shop attribution here even though `booking_capabilities` carries no retention window. Telemetry: `?handoff=` is on `CAPABILITY_QUERY_PARAMS`, pinned by that same test. |
| Arrival code (the QR drawn into `/s/[shopSlug]/trips/[id]/arrival-card`, typed back at the kiosk) | Hashed, expiring, revocable row in `booking_capabilities` (`purpose = 'arrival'`; same table/module as readiness, issue #1600) | Yes — call `revokeBookingCapabilities(db, { shopId, bookingId, purpose: "arrival" })`, and **this is the one purpose the holder can retire without asking anybody**: `/ready/[token]` carries a "Stop the code on a saved card" control that calls exactly that (`stopArrivalCodesFromReady`, `src/app/ready/[token]/actions.ts`, issue #1729), for the diver who cannot find the card they printed. It retires *every* live `arrival` row for the booking rather than one, because a code is minted per download and a plaintext token can never be re-derived from its hash — "the one on the card I lost" is identifiable by nobody, here or anywhere — and saving the card again mints a fresh one, which is the recovery path the copy promises. It is offered only where the booking already holds a live code (`hasLiveArrivalCapability`) and it answers a stranger's readiness token exactly as it answers the diver's, so it discloses nothing and is not an oracle. An operator revoking under pressure needs none of that: the same call, from a console, is the whole of it. A cancelled booking's or called-off trip's outstanding codes fail closed inside `verifyBookingCapability` like every other purpose, and the same per-purpose live cap applies, with a fresh code minted per download so the cap retires the oldest. **The one capability of this set that leaves on paper** — the card is a file the diver saves, prints and can forward — so two things bound it. Its life is the *departure*'s rather than the trip-anchored default: `arrivalCardExpiryFor` (`src/lib/booking-capabilities.ts`) is `startsAt + KIOSK_AHEAD_HOURS`, and `issueBookingCapability` keeps whichever of that and the trip bound is sooner, so a seat booked a season out no longer prints a credential live for the season plus a month. And what it buys is exactly what saying a surname at the kiosk buys — one diver's first name, their departure's title and time, the meeting point, and an arrival recorded against that booking — never boarding, and never the readiness surface: the kiosk's reader asks for `purpose: 'arrival'` (`bookingForArrivalCode`, `src/db/kiosk-check-in.ts`), so the readiness token that authorized the download is not a key to that door. Telemetry: this token never appears in a URL at all (it rides a QR into a saved file and is typed back by a wedge scanner), and the readiness token on the *download* URL is a `?booking=` value, which `redactCapabilityUrl` blanks on any path. |
| Recap link (`/recap/[token]`) | Stateless signed token, no stored row (`src/lib/recap-links.ts`) | Not individually — there is no row to revoke, and no ticket has moved this onto `booking_capabilities`. It does die on its own, though: every payload carries its own issued-at and stops verifying past `RECAP_TOKEN_MAX_AGE_MS` (180 days), so an exposed recap link is bounded rather than valid for the life of the booking id. **Deliberately no self-serve rescue**, unlike the waiver and readiness links (issue #850). Those reissue a *stored* capability, so the act is bounded by rows that already exist and by a live-link guard. A recap token has no row: "send me a fresh one" would mean minting a new signed token for a booking on the say-so of an expired one, with nothing to check it against and no way to revoke what it produced. A diver who needs their recap again asks the shop, which is a person deciding. **A dead recap link names that shop** and hands over its published contact details, as of issue #1119 — the booking tier of ADR 20260827-first-light decision 3, which the other three booking tokens were already in. What the page acts on is a *verified* signature, so a forged token still reaches the bare notice that names nobody; the split lives in `getRecapPageState` (`src/db/recap.ts`) rather than in the page, so no branch there can widen it. |
...
| Departures-board display link (`/board/[token]`) | Hashed, **non-expiring**, revocable row in `display_tokens` (`src/db/display-tokens.ts`, issue #1426) | Yes — revoke it from `shop/[shopSlug]/settings/display` (`revokeDisplayToken` sets `revoked_at`), and the screen stops showing the day on its next minute's refresh — the board re-verifies on every render and answers a revoked token exactly as it answers one that was never ours. Non-expiring for the calendar feed's reason: a TV on a wall that went dark after 60 days would be noticed by a diver before a staffer. What it exposes is deliberately narrow and closed by type (`BoardDeparture`, `src/db/departures-board.ts`): today's departure titles, times, sites, boats, the crew's stage word, an "n of capacity" count, the meeting point and the automated outlook — never a diver's name, phone, readiness or money; the crew line only on a link minted with `show_names`. An exposed one is a live read of the shop's day, not of any person, and revoking it is the whole remedy. |
| Self check-in kiosk link (`/check-in/[token]`) | Hashed, revocable, and the one row in `display_tokens` that **expires**: `purpose = 'check_in'` is stamped with an `expires_at` 180 days out (`CHECK_IN_LINK_TTL_DAYS`, issue #1609), renewed by a manager from the same settings page (`src/db/display-tokens.ts`, N-24) | Yes — revoke it from `shop/[shopSlug]/settings/display`, exactly as for the board link above; the tablet answers with its own refusal card on the next request. It is the same table and the same credential shape as the board's, and the **only** one of this set that lets its holder *write*: a lookup on it records an arrival against a real booking. That is why the purpose is matched inside `verifyDisplayToken`'s predicate — a board link handed to whoever mounts a TV must not open a surface that writes, and crossing the two is refused with the same `null` an unknown token gets. What it exposes on a hit is one diver's first name, their departure's title and time, and the meeting point, and to somebody who either already knew that diver's surname or holds the arrival code off that diver's own card (the `arrival` row above, issue #1600) — a scanned code reaches the same hit, which is why it is minted per booking and dies with the departure rather than being a reusable key; a miss, an ambiguous surname and a diver readiness will not clear are one identical sentence, so the screen cannot be walked to enumerate a roster. Lookups are rate-limited twice: per link (`RATE_LIMITS.kioskLookup`) and per caller before the token is verified (`RATE_LIMITS.kioskLookupByIp`), so guessing at the signature is not free. The expiry is the backstop for the one thing revocation does not answer — a tablet that leaves the counter with nobody left to revoke its URL. What it can never do is board anyone: nothing behind this token touches `roll_call_events` or anything the manifest reads. |
| Email-unsubscribe link (`/unsubscribe/[token]`) | Hashed, **non-expiring, non-revocable** row — two of them, `last_minute_list_unsubscribe_tokens` and `person_courtesy_email_unsubscribe_tokens` (`src/db/last-minute-list.ts`, `src/db/courtesy-email.ts`); neither table has an `expires_at` or a `revoked_at` column | No. Lowest stakes of the set — the capability is "stop sending this address these emails", and the page reveals only the shop's name — but it is also the only one with no expiry *and* no revocation, so an exposed one works forever. Its redaction, once the one gap in `redactCapabilityUrl`, is now covered and pinned; see [the gap the redaction itself still has](#the-gap-the-redaction-itself-still-has) for what is left. |
| Diver shelf link (`/shelf/[token]`) | Hashed, expiring (a year — `SHELF_TOKEN_TTL_MS`), revocable row in `person_shelf_tokens` (`src/db/person-shelf-tokens.ts`) — **the one bearer surface anchored to a person** rather than to a booking, so its life is not derived from any trip | Yes — `revokeShelfTokens`, reached both by the diver's own "Forget this phone" (`src/app/shelf/[token]/actions.ts`) and by erasure (`src/db/anonymize.ts`); `MAX_LIVE_SHELF_TOKENS` (20) live per person, oldest revoked past that. **The one credential of this set that is actually pruned**: `person_shelf_tokens: 90` in `src/lib/retention.ts`, ninety days past the day it stopped working, so a dead shelf link eventually stops attributing anything at all. The stakes are the highest of the diver-facing set — one person's whole file at one shop: the cards they hold, where their release stands, their sizes. And it **mints booking handoffs**: `src/app/shelf/[token]/actions.ts` calls `issueBookingHandoff`, so an exposed shelf link is not one read but a renewable source of ten-minute booking credentials for up to a year. That is the reason this row cannot be the one that stays missing. |
| Gift link (`/gift/[token]`) | Stateless signed token, no stored row (`src/lib/gift-links.ts`) — the recap link's shape, and therefore the recap link's exposure | Not individually — there is no row to revoke. It dies on its own at 180 days (`GIFT_TOKEN_MAX_AGE_MS`). **That it outlives the receiver's `claim` capability is the design, not an oversight**: `claimSeatRecord` (`src/db/seat-claims.ts`) revokes every `booking_capabilities` row on the booking the moment the seat changes hands, which is exactly the moment the giver's own link becomes the thing they still need — so it cannot be a capability row. Purpose-prefixed (`GIFT_PURPOSE`), so a gift token fails verification on `/recap` and a recap token fails on `/gift`. Signing key: `GIFT_LINK_SECRET` when the deployment sets one, otherwise an HKDF derivation over `AUTH_SECRET` (`giftSecret`) — **check which one is set before rotating anything**, the same warning the recap row carries, because rotating `AUTH_SECRET` on a deployment that sets `GIFT_LINK_SECRET` invalidates no gift link while signing out every staffer. What it exposes is closed by the `GiftGiverView` type (`src/db/gifts.ts`) and asserted key by key in `gifts.test.ts`: the shop's name, slug, locale, timezone and already-published contact details, the departure's title and times, and the course or site that would make another departure the same gift — never the receiver's readiness, and no diver contact detail. |
| Hidden-course preview (`/s/[shopSlug]/courses/[slug]?preview=[token]`) | Stateless signed token, no stored row (`src/lib/course-preview-gate.ts`, issue #1735) — the recap and gift links' shape, HKDF over `AUTH_SECRET` under its own purpose so it verifies as nothing else and nothing else verifies as one | Not individually — there is no row to revoke. It dies on its own at **ten minutes** (`COURSE_PREVIEW_TTL_MS`), and it is minted only on the tap, by a route that has just proved the caller is that shop's live staff (`shop/[shopSlug]/courses/[slug]/preview`), so it is never rendered into a page and never sits in a screenshot. **It is the least of this set by a distance, and the row is here to say why rather than to promise a lever.** It is not an authorisation and cannot be made into one: possession gets a reader past the edge's refusal and no further, because the public course page's own `isLiveShopStaff` is live, per-shop, and still the only gate (`src/app/s/[shopSlug]/courses/[slug]/page.tsx`). A stranger holding a valid token reads the same `notFound()` they read without one. What it *does* carry is one fact — **this shop is holding a draft at this slug** — which is the entire disclosure the capability exists to prevent, so `preview` is on `CAPABILITY_QUERY_PARAMS` and `course-preview-gate.test.ts` imports the constant to hold the two together. Scope is the shop slug and the course slug the route resolved, both inside the signature, so one draft's link opens no other draft and no other shop. If one is known to be out and the ten minutes matter, there is nothing to revoke: publish the course or leave it, and it is moot either way. |

**What keeps this table complete.** Its coverage is keyed on three lists at
once — `CAPABILITY_ROUTE_PREFIXES`, `CAPABILITY_QUERY_PARAMS` and the
`CapabilityPurpose` union (`src/db/booking-capabilities.ts`) — never on the
purpose union alone. A sweep that checks only the union re-creates the same
omission one table over, because the rows above also cover `waiver_records`,
...
It is closed, and since 2026-08-27 it is closed through a **supported hook rather than an
undocumented global**. `trackEvent` (`src/lib/analytics.ts`) passes the SDK a `beforeSend` that runs
the page URL through the same `redactCapabilityUrl` the browser SDKs use, and returns `null` —
withholding the event entirely — if it ever cannot. That option does not exist in any published
`@vercel/analytics`: it is `patches/@vercel__analytics@2.0.1.patch`, carrying
[vercel/analytics#208](https://github.com/vercel/analytics/pull/208) verbatim until it is released
(issue #622).

What it replaced was a delegating shim over `globalThis[Symbol.for("@vercel/request-context")]`,
which depended on three undocumented details of Vercel's runtime — the symbol, the `{ get(): { url } }`
shape, and the writability of the slot. The last of those cost a day of thrown errors and one 504
before anyone noticed, found in a log drain rather than by a check. That is the argument for the
patch over the shim: a wrong guess about the SDK's *own* source fails when the patch stops applying,
which is an install-time error, where a wrong guess about the runtime's private globals fails
silently by quietly resuming the leak.

Three consequences worth carrying forward:

- **The definition of a capability URL lives in `src/lib/capability-urls.ts`**, re-exported by
  `src/app/observability.ts`. Both halves of the app redact through one function, which is the only
  way they can be relied on to agree. `src/lib` may not import `src/app`, which is why it lives there.
- **The hook covers more than the shim did.** The shim edited `requestContext.url`; `beforeSend`
  ...
  not reach.
- **The patch is pinned to `@vercel/analytics@2.0.1` on purpose.** Bumping the dependency makes the
  patch key stop matching and `pnpm install` say so, which is the intended alarm: whoever bumps it
  checks whether the release carries `beforeSend` natively, and if it does, deletes the patch and
  changes nothing else. `src/lib/analytics.test.ts` proves the redaction against the hook's contract
  rather than against the SDK's source, so it keeps working either way.
...
because unlike a browser SDK it will not come with a `beforeSend` to add afterwards. That covers a Meta
...
2. Filter on `waivers/`, `ready/`, `recap/`, `verify/`, `reset-password/`,
   `invite/`, `claim/`, `calendar/`, `unsubscribe/`, `board/` and `check-in/` as **path**
   prefixes, and
...
- **A dead readiness or claim link still names its shop.** Deliberate for
  `/ready` since issue #801 and for `/claim` since ADR 20260827-first-light,
  decision 3 wrote the dead-link law's two tiers down: both are links a diver is
  *sent*, so reaching a dead one is the ordinary case, and "ask your dive shop"
  without saying which shop is a dead end. What it discloses is the shop's own
  published name and contact details, to a bearer who had to be given the token,
  and the resolvers (`staleBookingCapabilityForToken`, and `getClaimPageState`
  in `src/db/seat-claims.ts`, whose return type carries four shop columns and no
  booking) read nothing else. It is **not** bounded by revocation or expiry, and
  the row is never pruned — so treat it as permanent when assessing an exposed
  URL. The claim link makes that reach further than the readiness one in
  practice: a claim is revoked the instant the seat is taken, so *every* spent
  claim URL is a permanent shop attribution. A token that resolves to no
  capability row at all still names nobody, on either page.
...
**The redaction gap this section recorded is closed.** `"unsubscribe"` is on
`CAPABILITY_ROUTE_PREFIXES` (`src/lib/capability-urls.ts`) and the redaction is
pinned by a case in `src/app/observability.test.ts`, whose own doc comment calls
itself "the guard that would have caught `/unsubscribe/[token]`". So the raw
token no longer reaches Analytics, Speed Insights or Sentry.

What is still true is the storage, and it is why the row above answers "No":
two token kinds share the route (a last-minute-list entry's opt-out and a
person's courtesy-email opt-out) and neither table carries an `expires_at` or a
`revoked_at`, so an exposed one works forever. The access-log residual every
other capability shares is also unchanged.

The heading stays, and so does the lesson under it, because a list that
silently omits an entry is how the `confirm` token stayed unprotected through
the first CR-001 fix — and that is precisely what happened twice more while
this section sat here saying it. `handoff`, `shelf` and `gift` were all live
and missing from the table below (issue #1730). The sweep rule under that table
is the answer to the *next* one.
