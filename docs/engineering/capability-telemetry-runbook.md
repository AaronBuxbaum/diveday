...
| Self check-in kiosk link (`/check-in/[token]`) | Hashed, **non-expiring**, revocable row in `display_tokens` with `purpose = 'check_in'` (`src/db/display-tokens.ts`, N-24) | Yes — revoke it from `shop/[shopSlug]/settings/display`, exactly as for the board link above; the tablet answers with its own refusal card on the next request. It is the same table and the same credential shape as the board's, and the **only** one of this set that lets its holder *write*: a lookup on it records an arrival against a real booking. That is why the purpose is matched inside `verifyDisplayToken`'s predicate — a board link handed to whoever mounts a TV must not open a surface that writes, and crossing the two is refused with the same `null` an unknown token gets. What it exposes on a hit is one diver's first name, their departure's title and time, and the meeting point, and only to somebody who already knew that diver's surname; a miss, an ambiguous surname and a diver readiness will not clear are one identical sentence, so the screen cannot be walked to enumerate a roster. Lookups are rate-limited per link (`RATE_LIMITS.kioskLookup`). What it can never do is board anyone: nothing behind this token touches `roll_call_events` or anything the manifest reads. |
