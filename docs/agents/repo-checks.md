...
The departure-buffer one (`scripts/check-departure-buffer.mjs`) refuses two shapes outside `src/lib/trips.ts`: an offset added to a `startsAt`/`endsAt` on a line that also *compares*, and any `*_BUFFER_MS` declaration. Both mean the same thing — somebody asked "has this sailed?" without going through `hasSailed()` / `hasReturned()`.
