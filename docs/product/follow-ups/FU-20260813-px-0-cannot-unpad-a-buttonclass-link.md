# FU-20260813-px-0-cannot-unpad-a-buttonclass-link — Two marketing links pass a `px-0` that Tailwind ignores
...
Two call sites believe otherwise, and both render a link that sits 16px right of the copy it
belongs under:

- `src/app/page.tsx:227` — `buttonClass({ variant: "link", className: "px-0" })`
...
right of the heading above it, which is how I found this). It is fixed there with `-ml-4` — the
...
three call sites reaching for the same workaround says the wrapper is missing a way to say it,
...
of the same amount, so the label's optical left edge is the container's. Then move the three sites
...
Two call sites rely on it and render a link indented 16px past the copy above it:
src/app/page.tsx:227 and src/app/pricing/page.tsx:281. src/app/product/page.tsx works around it
with `-ml-4`, and src/components/JumpNav.tsx does the same with `-ml-3`.
...
docs/design/forms-and-controls.md, and move all four call sites onto it (the two above, the
