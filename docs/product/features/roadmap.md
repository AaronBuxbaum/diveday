...
- **15a** — `/about` gains a "Who's here" band directly under the hero: three short paragraphs in
  team voice, the background as one passing clause, no ledger, no signature; "spec" leaves
  `founderP3`. Waits on H-66 (a).
- **15b** — `/` gains "The short version" between the records diptych and the close: the whole
  case in five plain sentences, the price interpolated, one link to `/about` as its only control.
- **15c** — the support sentence becomes "one of us reads it" in its five places, never with a
  response time. Waits on H-66 (b).
- **15d** — the voice pass across every marketing page, held to the eight tells on the ledger.
