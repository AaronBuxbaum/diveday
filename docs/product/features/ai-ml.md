...
- **Priority**: medium
- **Effort**: M
- **Prompt**: The catalog states a progression on each course itself — `minimum_certification_level`, the field the staff roster already orders by since ADR 20260805-remove-certification-paths retired the shop-built path tables — and the recap page (`src/app/recap/[token]`, data in `src/db/recap.ts`) is the highest-intent diver moment the app owns. Add a query in `src/db/courses.ts` that, given a person id, joins their highest `certifications` level against the shop's active catalog and returns the lowest-ranked course they haven't taken and whose `minimum_certification_level` they already meet. Render at most one suggestion on the recap page as a low-key card linking to the public course page (`shop/[shopSlug]/courses/[slug]`), with copy in `src/i18n/locales/<locale>/diver.json` and a `DiverIntlProvider` already above it (verify — a missing provider blanks the whole page). This is a deterministic join, not collaborative filtering: with one shop's enrollment volume, "the next rung your card already opens" is strictly better than any learned recommender, and the module doc should say so. Guidance never gates: admission stays on the course's own `minimum_certification_level` check at booking time. Respect the bearer-token page rules in `docs/engineering/capability-telemetry-runbook.md` — no new data exposure beyond this booking's diver, and no structured data on the token page.
- **Verification**: `src/db/courses.test.ts` cases: uncertified diver → entry course, mid-catalog diver → next course up, fully-certified → nothing, hidden course → nothing. E2e assertion on the recap spec that the card shows for the seeded diver; screenshot added to `e2e/visual.spec.ts`; `pnpm check` green.
