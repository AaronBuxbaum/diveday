...
| H-03 | Chosen | Product owner + qualified legal reviewer | Is the current typed name + explicit consent + timestamp sufficient for the intended release, or is a specialist e-signature provider required? | Accepted assurance level, required provider criteria (if any), and rollout boundary. | **Chosen 2026-07-24:** keep the local signature provider (typed name + explicit consent + timestamp) for now — no specialist e-signature vendor for the current slice. The baseline already ships, so no new implementation is required; the vendor-adapter option stays available behind the provider seam if later needed. Whether typed consent is legally *sufficient* remains open and rides with H-01/H-02 and V-03. Baseline documented in the [provisional defaults](#waiver-and-signature). |
...
| H-06 | In progress | Product owner + dive operations lead | What is the initial gear policy for sizing, preferences, staff assignment, and substitutions? | Required measurements/preferences, who may override a request, and the safe fallback when a size is unavailable. | **Chosen 2026-07-24 (default-kit membership):** the **dive computer is now part of the default rental kit** — it defaults on for every diver and is folded into the priced core "set" bundle (`src/lib/rentals.ts`; only the GoPro remains an opt-in add-on). Shipped with tests and the [rental gear request](#rental-gear-request) note updated. Consequence to note: a diver who declines the computer no longer completes the full six-item set, so they are quoted per piece rather than the cheaper set price. Still open (the rest of H-06): measurement method, substitution authority, and the safe fallback when a requested size is unavailable — the booking-level rental request otherwise uses the [standard provisional set](#rental-gear-request). *(This decision arrived labelled "H-05" but its content — the default kit — belongs to this gear-policy row; register H-05 is the offline-manifest thresholds.)* |
...
| H-12 | In progress | Product owner + finance owner | Is the public founding-shop price, commercial term, support promise, and multi-location policy correct? | Approved monthly price, billing cadence, support/onboarding commitment, taxes/fees policy, and public contact/contract flow. | **Approved 2026-07-24 (price):** the founding-shop price stays **$99 per location / month** for now (early access, still moving). This matches the figure already published from `src/lib/marketing.ts` (the source of truth) via the [pricing boundary](marketing.md#pricing-boundary), so no code change was needed. Still open: billing cadence, support/onboarding commitment, taxes/fees policy, and the public contact/contract flow. |
...
- **Waiver evidence:** the first release keeps immutable local records with typed consent; it does
  not claim cryptographic non-repudiation. The signature provider is settled — local for now (H-03,
  2026-07-24) — so the unresolved policy work is H-01 and H-02.
...
- **Must verify (H-01–H-02):** jurisdiction, approved template and medical questions, age/guardian
  ...
  *sufficient* still depends on this legal review.
...
- **Starting rental set:** BCD, regulator, wetsuit, mask/fins, weights, dive computer, and tank.
  Per the H-06 decision of 2026-07-24 the **dive computer is now part of the default kit** (on by
  default, included in the priced core "set" bundle); the GoPro is the one opt-in add-on. The
  request asks for BCD/wetsuit size, boot/fin size, usual weighting, and notes.
...
- **Must verify (H-06):** shop inventory packages, thickness/temperature guidance, measurement
  ...
  is not available. (Computer policy is decided: it is in the default kit.)
...
| 2026-07-24 | Recorded three product-owner decisions. **H-03:** keep the local signature provider for now (baseline already ships; legal sufficiency still H-01/H-02). **H-06:** the dive computer joins the default rental kit and the priced core "set" bundle — shipped in `src/lib/rentals.ts` with tests, glossary, and the rental-request note updated (arrived labelled "H-05"; belongs to the gear-policy row). **H-12:** the founding-shop price is approved at $99 per location/month for now, matching the published `src/lib/marketing.ts` figure; billing cadence, support/onboarding, taxes, and contract flow still open. | Product team |
