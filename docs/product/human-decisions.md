...
- **Cancellation window (opt-in, shipped, declarative):** a shop may set `cancellation_window_hours`
  (hours before departure a diver can cancel for a refund). It is *shown* to divers at booking and
  on the confirmation, and to staff as a "refund-eligible until" cue on paid seats. It moves no
  money — refunds remain staff-run. Off by default (null = no stated window); DiveDay ships no
  default window.
...
- **Must verify (H-07):** the deposit-amount and cancellation-window *values* a shop should be
  guided toward (DiveDay ships none), whether refunds should ever be automated inside the window,
  percentage vs. flat deposits, tax treatment, any platform fee, and whether unpaid bookings should
  auto-expire.
