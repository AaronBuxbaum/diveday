...
| 2026-07-24 | **Closed H-12**, per the rollout plan's recommendation: kept the $99 flat per-location/month price (no code change — already the published `src/lib/marketing.ts` figure) and added two founding-cohort commercial terms — a **two-year price lock** and **founder-direct, same-day-response support**. Propagated the closure into `marketing.md`'s claims policy and `roadmap.md`'s pricing-posture gate; publishing the two new terms as public page copy is a separate follow-up (not yet implemented). Billing cadence, taxes/fees, and the public contract/contact flow remain open but non-blocking. | Product owner |
