...
| H-03 | Ready | Product owner + qualified legal reviewer | Is the current typed name + explicit consent + timestamp sufficient for the intended release, or is a specialist e-signature provider required? | Accepted assurance level, required provider criteria (if any), and rollout boundary. | **Provider sub-decision recorded 2026-07-24:** keep the local signature provider (typed name + explicit consent + timestamp) for now — no specialist e-signature vendor for the current slice; the vendor-adapter option stays available behind the provider seam if a later review requires it. **This does not clear the gate:** whether typed consent is a legally *sufficient* assurance level for the intended release is still undecided and must be signed off with H-01/H-02 and V-03 before production. Row stays **Ready** until that assurance outcome is recorded. Baseline documented in the [provisional defaults](#waiver-and-signature). |
...
| H-14 | Chosen | Product owner | The [codebase review](../architecture/decisions/20260724-staff-session-and-capability-migration-policy.md#context)'s "Human-owned decisions" item 1: current ADRs let every staff role reach nearly every staff surface, while product language implies owners/managers configure money and crew/captains run operations. Does DiveDay need real role boundaries on payment settings, refunds, waiver templates, diver deletion, and trip configuration? | **Decided 2026-07-24: yes**, role boundaries are needed on those five surfaces. Not yet implemented — needs a follow-up ticket to design the actual role/permission matrix (which role(s) may reach each of the five surfaces) and enforce it in `src/lib/authz.ts` and the relevant server actions, with `security-reviewer` and `dive-domain-expert` review before merge per AGENTS.md's hard rules. | Add to `roadmap.md` as an implementation ticket; the decision alone does not change any code. |
