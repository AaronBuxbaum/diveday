...
| A11Y-04 | Roll call, live and offline manifests (2026-08-23) | Haptics stay **on** by default — a 10 ms tick when a tap lands, `[40, 40, 40]` when one is refused, and `[100, 50, 100, 50, 200]` when everyone is aboard — with a per-device switch beside boat mode rather than an opt-in | Somebody with tremor or a vestibular condition meets half a second of buzzing before they find the switch, and they have to find it on each device. Opt-in was the alternative and it would have cost the wet-deck crew the whole channel, since nobody turns on a feedback layer they have never felt. `prefers-reduced-motion` does not reach vibration, so the app's motion kill-switch cannot carry this | A crew member reporting the completion pattern is too much. The fix then is a shorter completion pattern or honouring `prefers-reduced-motion` for the long ones, not moving the tick behind an opt-in |
