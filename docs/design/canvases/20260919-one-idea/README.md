...
| 23g — the rest, one family per session: courses, gear, money, reviews, staffing, and Settings behind the shop's name | shipped | `src/lib/staff-destinations.ts` | `src/lib/closeout.test.ts`'s `dayTakings`, `src/app/shop/[shopSlug]/settings/settings-doors.test.ts`, `src/lib/staff-destinations.test.ts`'s "lights Today for the gear register", `e2e/day-close.spec.ts`, `e2e/staff-nav.spec.ts`'s "the gear register lights Today" |
...
- **gear** was filed under the wrong time, and the mapping said so itself (#1937). Slice 23b moved it
  from `navGroup: "daily"` to `place: "shop"` — the place with no hour in it, defined by living
  behind the shop's own name — while leaving the row's own comment reading "chasing returns is the
  day's rhythm, **not** configuration". It never lived behind that name either: Settings carries a
  `SettingsDoorRow` for the site library, the waiver template, promo codes, the team and the
  calendar feed, and none for the fleet, so on a morning with nothing overdue the register was
  reachable by the search alone — the finding ADR 20260813-more-is-the-shops-other-door retired an
  earlier nav on. It is `place: "day"` now, beside `divers`, `checkIn`, `walkIn` and `inbox`, which
  are whole lists too and are the day's for the same reason. A wetsuit goes out at eight and is
  chased at four.
- **reviews** needed nothing, and that is the finding rather than an absence of one. `place:
  "season"` is the ADR's own sentence verbatim — "what the days added up to: the money, the month,
  the reviews" — every row names the departure and the date it was dived and links to both the trip
  and the diver, which is the idea's "reached from the object it belongs to", and its pending work
  is a Today row rather than a nav badge.

**The half nothing held.** `place: "shop"` is a claim about *where a surface is reached*, and no
test tied it to the doors that make it true — which is exactly how it came apart on the day it was
written. `settings-doors.test.ts` now asserts that Settings carries a row for every `shop`
destination **and for no other**: the first direction is the gear bug, the second is Settings
growing back into the nav of nouns 23b deleted.
