...
| 23c — the departure: the hour page with the strip (I), the hull with its seats (II), the voyage on the chart (III); the four trip tabs one page; the roll call's one tap untouched beneath | in progress | `src/app/shop/[shopSlug]/trips/[id]/_components/TripHull.tsx` — the boat drawn above its roster, from `boats.hull_color` and `src/lib/hull.ts`; the hour page and the four tabs are still ahead | `src/lib/hull.test.ts` (the canvas's own outlines, verbatim), `src/app/shop/[shopSlug]/trips/[id]/_components/TripHull.test.tsx`, `e2e/trip-hull.spec.ts` |
