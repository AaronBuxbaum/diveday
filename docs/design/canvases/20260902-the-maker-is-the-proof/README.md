...
| `AboutBuilder.dc.html` | `/about` at 1280: the shipped header and hero, then the new "Who builds it" band — the signed story beside the three-row ledger — and the head of the rules band that follows |
...
| --- | --- | --- | --- |
| 15a — `/about` "Who builds it" band, signed, with the ledger; "spec" leaves `founderP3` | open | — | — |
| 15b — `/` echo band between the records diptych and the close | open | — | — |
| 15c — the support sentence in five places (gated on H-66, Option B) | open | — | — |
...
`conversion-reviewer` pass. 15a and 15c carry a real person's name and a commitment, so both wait on
...
The name is Aaron Buxbaum. No home location, no certification year, no second person named. Everything
written in the first person — three paragraphs and the three ledger lines — is a draft in the
owner's voice for him to confirm, reword or strike; the self-driving line is flagged for him in
particular, since it is true and says DiveDay is not his day job. The shop on every mockup is the
