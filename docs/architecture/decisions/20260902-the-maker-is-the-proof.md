# 20260902-the-maker-is-the-proof — Put the founder's background back on the marketing pages, as proof rather than a résumé
...
staff and his own work background (ex-Google). A second note in the same session: the copy should
sound less like it was written by a machine.
...
1. **The site has no maker.** Every page argues the product's safety — safe to run the boat on,
   safe to leave — and none argues the person. The "From the founder" band on `/about` carries no
   name. The confirmed background (a software engineer who worked on Google Maps, helped build a
   biotech company that went public, and works on self-driving cars — confirmed 2026-07-25) came
   off the page on 2026-08-05 because a "Who builds it" credential block did not tell the story.
   Against the field DiveDay is actually selling into — a three-person studio, a private-equity
   POS, and a back-office PC — the maker is the differentiator, and today's pages spend it nowhere.
2. **The pages argue insurance, not desire.** The product's bet is delight
   ...
   Nothing says why this one will feel different, and the quality bet needs a reason to be believed.
   The credential is that reason.
3. **Support is described as a mailbox.** "A real person reads it" is on every page and any rival
   ...
   volume. The one support sentence no incumbent can write is *who* answers. It was retired on
   2026-08-05 (H-12) because "founder-direct" read small — beside "One person owns every line of
   code", a sentence about headcount. The subject of the sentence was the problem, not the fact.
...
2. **`/` gets one echo, and no new door in the hero.** A band between the records diptych and the
   ...
   one sentence, one link to `/about` at link weight, and a hairline ledger where a mockup would
   sit whose labels are the three confirmed facts and whose values are **shipped product facts a
...
4. **"Ex-Google" is used in prose once, on `/about`, as "worked on Google Maps"** — the product a
   ...
   description; it is never a heading on `/`. Never a Google logo
   or any logo wall (it would read as an affiliation Google did not grant), never an eyebrow or a
   hero headline, never "Google-grade", never beside a home location, a customer count, or a
   response time. The hero headline of every page stays about the buyer's position; the maker is
   proof beneath it, never the pitch.
...
- **Leave the biography off, as decided 2026-08-05.** The block that came off was a résumé; the
  decision was about the telling, not the facts, which stayed confirmed. A résumé and a signed
  story are different objects, and the page without either has no maker.
- **"Ex-Google" as an eyebrow or in the hero.** A startup-deck register, and it makes the maker
  the pitch. The rule that the headline is about the buyer's position was learned four times on
