...
| `trip_last_minute_promo_recipients` | `email` → a unique unusable address. The row stays: it is the shop's own record that a deal went out to N people on a departure, and with the address gone it is no longer a fact about a person. `person_id` stays for the reason `course_inquiries`' does — it points at a row that has itself been erased, and keeping it is what makes a replayed erasure reach the same rows. Found by the person-reachable sweep issue #1607 asks for; the table is in the portable export bundle, so the address was leaving the shop with every deal the diver had ever been offered. |
