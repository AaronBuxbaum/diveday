...
| Table | Treatment |
| --- | --- |
| `people` | `full_name` → `[anonymized]` sentinel (NOT NULL); `email`, `phone`, `emergency_contact_*`, `date_of_birth`, `dive_insurance`, `locale`, `courtesy_email_opt_out_at`, `no_certification_declared_at` → null; `deleted_at` set; `anonymized_at`/`anonymized_by_person_id` stamped. Email must go to **null**, never a sentinel — `people_shop_email_unique` is a partial unique index on `lower(email)`, and a shared sentinel address is both a collision hazard and not an erasure. |
...
| `activity_events` | the row is rewritten to `ACTIVITY_REDACTED` — `code` → `redacted`, `params` → `{}` — so the line reads `[redacted]` in the reader's own language; the row, its actor, and its timestamp stay. (Until 20260910-the-trail-is-a-code-and-names this was a free-text `message` column replaced with the literal `[redacted]`; the outcome is unchanged, and redacting the payload alone would have left the verb standing.) Swept four ways — by booking, by actor, by `subject_person_id` (the person a line is *about* where no seat carries it: a note written on the diver's record), and by **matching the stored name on word boundaries** (`~*` with `\y`, case-insensitive, over `params` as text) as a backstop for a row that names the diver while hanging off someone else's seat or none at all. The name match used to be the *only* handle on a record-scoped note, whose subject lived solely inside the message text; `subject_person_id` carries it outright as of #615. The first three are exactly the predicate `pagedDiverActivity` reads under (`src/db/operations.ts`) — they move together or not at all, so what a shop can read about a person and what an erasure destroys stay one set by construction. See "Bounding the name match" below: the match is anchored and has a minimum length, and its match count is logged. |
...
The `activity_events` name sweep matches the stored name against **every row in the shop**, so
