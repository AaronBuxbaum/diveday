...
| [product/follow-ups/](product/follow-ups/README.md) | The agent inbox: one file per idea, question, risk, or deliberately-skipped cleanup an agent left for a human to triage, each ending in a prompt that can be run cold | you finish a change with a thought you did not act on (file it there, in the same commit); the human accepts it into the roadmap/backlog/decision register or declines it, and deletes the file. Its [waiting/](product/follow-ups/waiting/README.md) room holds the entries nobody here can move — an upstream release, a third party's answer, numbers that need traffic — so the inbox stays things you can act on today |
