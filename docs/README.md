...
| [agents/issue-tracker.md](agents/issue-tracker.md) ("Filing a follow-up") | The agent inbox: one GitHub issue per idea, question, risk, or deliberately-skipped cleanup an agent left for a human to triage, each ending in a prompt that can be run cold | you finish a change with a thought you did not act on (file it as a `needs-triage` issue, list its number in the PR); the human accepts it into the roadmap/backlog/decision register or declines it, and closes the issue. `waiting-on-external` marks the ones nobody here can move — an upstream release, a third party's answer, numbers that need traffic |
| [agents/repo-checks.md](agents/repo-checks.md) | Why each `pnpm check:repo` guard exists — what it protects, the incident behind it, and its escape hatch. Read the one section your red check names; the guards say everything else themselves | a guard is added, or one's reasoning changes |
...
| [product/pilot-kit/](product/pilot-kit/README.md) | The paper the founder carries into Phase 1 recruiting: design-partner one-pager, Florida call-list template, first-call script, printable V-02 run sheet | a recruiting artifact is added or a conversation proves one wrong (gate *status* still lives only in human-decisions.md) |
...
| [architecture/performance-budgets.md](architecture/performance-budgets.md) | The per-route client-JS budgets `scripts/perf-budget.mjs` enforces and the reasoning behind the numbers | a budget value or the measurement method changes |
| [architecture/boat-resource-model-dossier.md](architecture/boat-resource-model-dossier.md) | Design dossier behind the **Proposed** boat/buyout-charter ADR: inventory, criteria, the three candidate designs and judge scores | the boat-model ADR moves status or the design space is re-opened |
| [architecture/aws-migration-dossier.md](architecture/aws-migration-dossier.md) | Every vendor DiveDay still buys from that is not AWS, its AWS replacement, and each swap's benefit/cost/risk as numbered `AWS-n` rows awaiting an owner yes/no ([H-45](product/human-decisions.md#decision-register)) | a row is decided (record it on H-45, and open an ADR if it is hard to reverse), a vendor's pricing or capability changes, or a coupling in the inventory moves |
| [architecture/infrastructure-costs-and-risks.md](architecture/infrastructure-costs-and-risks.md) | Provider spend inventory, future cost growth vectors, step-function plan cliffs, architectural single-points-of-failure, and live cost telemetry automation | pricing, provider usage tiers, infrastructure topology, or cost telemetry methods change |
...
| [design/settled-questions.md](design/settled-questions.md) | The register of things that look like a design defect, were investigated, and are right — each row pointing at the file whose comment carries the reasoning | before a design sweep, and during one whenever something that looks wrong turns out not to be |
| [design/surfaces.md](design/surfaces.md) | Where the holistic pass is written down — one entry per significant surface, five answers, nothing else | before reviewing or reshaping a surface, and when building a significant new one |
| [design/design-artifacts.md](design/design-artifacts.md) | How a design drawn *before* the code enters this repo: what a canvas may claim (pictures argue, the ADR decides, code obeys the ADR), where its artboards live, why it is dated and superseded rather than freshened, and what must happen when it ships | before drawing, editing, or reading a design canvas — and before treating any mockup in this repo as normative |
...
| [engineering/ses-email-runbook.md](engineering/ses-email-runbook.md) | Setting up SES sending and the delivery-outcome webhook; how DiveDay's own hosted addresses and DMARC are configured; what to check when mail doesn't arrive | the email envelope, webhook events, or the sending domain changes |
...
| [engineering/sms-delivery-receipts-runbook.md](engineering/sms-delivery-receipts-runbook.md) | How an SMS delivery receipt reaches DiveDay (CloudWatch → forwarder → SNS topic → webhook), switching it on, and reading the pipeline backwards when receipts stop | the receipt pipeline, its retention, or the status mapping changes |
...
| [engineering/cloudwatch-observability-runbook.md](engineering/cloudwatch-observability-runbook.md) | Where the app's structured log lines go, the seven counts and three Core Web Vitals that raise an alarm, the dashboard and saved queries, CloudWatch RUM's deliberate narrowings, what it all costs, and what to check when something doesn't arrive | a signal, a threshold, the retention window, the RUM configuration, or the cost picture changes |
| [engineering/infrastructure-runbook.md](engineering/infrastructure-runbook.md) | Provisioning AWS S3, users, and resources using AWS CDK: login, bootstrapping, synthesis, deployment, and the credentials secret | CDK stack, context, or credentials changes |
| [engineering/manual-actions.md](engineering/manual-actions.md) | Every step `cdk deploy` cannot perform, with what to run and where each result goes. **Generated** from the registry in `infra/lib/infra-stack.ts` | never by hand — edit the registry and run `pnpm test infra -u` |
...
| [engineering/cost-guardrails-runbook.md](engineering/cost-guardrails-runbook.md) | Every provider ceiling DiveDay watches, what each one does when hit (bills, suspends, or silently drops), what the daily usage poll can and cannot see, and the escalation | a ceiling value, a probe, or a provider's overflow behaviour changes |
| [engineering/realtime-manifest-events-runbook.md](engineering/realtime-manifest-events-runbook.md) | How many Neon direct connections the manifest SSE/LISTEN channel holds, what happens at the ceiling, what to watch, and what a viewer loses when a stream is refused | the connection lifecycle, the subscriber ceiling, or the log lines change |
| [engineering/backup-and-restore-runbook.md](engineering/backup-and-restore-runbook.md) | Neon PITR and branch-from-timestamp restore, the scheduled per-shop logical export to S3 and its two known gaps, media object-storage posture, and the quarterly restore test plus its log | the backup destination, the export seam's coverage, Neon's retention window, or a restore test result changes |
...
| [archive/comprehensive-review-20260802.md](product/archive/comprehensive-review-20260802.md) | 2026-08-02 ten-lens whole-app review (product, architecture, security, dive-domain safety, data model, payments, testing, i18n/UX/a11y, marketing, operations); dissolved and archived 2026-08-07 across five reconciliation passes — every engineering finding shipped, and the human-decision register plus two small buildable leftovers moved into [product/human-decisions.md](product/human-decisions.md) (H-31–H-44) and [product/features/roadmap.md](product/features/roadmap.md#smaller-follow-ons-live-with-their-adrs) |
