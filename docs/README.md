...
| [engineering/whatsapp-cloud-api-runbook.md](engineering/whatsapp-cloud-api-runbook.md) | Connecting a shop's own WhatsApp Business number through Meta's Cloud API: platform setup, what a shop needs approved, the test send, Meta error codes, and rotating the credential-sealing key | the adapter, the template contract, or the connect flow changes |
