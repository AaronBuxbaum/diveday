...
export async function loadShopExportBundleInput(
  ...
  const roleRows = peopleRows.length
    ? await db
        .select()
        .from(personRoles)
        .where(inArray(personRoles.personId, [...personName.keys()]))
    : [];
  ...
  const tripIds = [...tripTitle.keys()];

  const tripDiveRows = tripIds.length
    ? await db
        .select()
        .from(tripDives)
        .where(inArray(tripDives.tripId, tripIds))
  ...
  const assignmentRows = tripIds.length
    ? await db
        .select()
