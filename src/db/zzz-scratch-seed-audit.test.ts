...
describe("seed audit", () => {
  it("lists every seeded trip's dives and sites", async () => {
    ...
    for (const row of rows) {
      ...
      entry.dives.push(`${row.diveNumber}:${row.siteName ?? "—"}${row.diveTitle ? ` (${row.diveTitle})` : ""}`);
      byTrip.set(row.title, entry);
    }
    const lines: string[] = [];
    for (const [title, entry] of [...byTrip].sort()) {
      const distinct = new Set(entry.dives.map((d) => d.split(":")[1]?.split(" (")[0]).filter((n) => n && n !== "—"));
      const blanks = entry.dives.filter((d) => d.includes(":—")).length;
      const flag = blanks > 0 && distinct.size > 0 ? "  <<< MIXED" : blanks === entry.planned ? "  <<< NO SITES" : "";
      lines.push(`${title} | planned=${entry.planned} sites=${distinct.size} blank=${blanks}${flag}\n    ${entry.dives.join(" | ")}`);
    }
