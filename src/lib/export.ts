...
export const EXPORT_FILE_NOTES = {
  ...
  "trip_invitations.csv":
    "Staff outreach attached to a departure without claiming a seat: the source request or diver, the contact snapshot, and whether staff recorded an invitation attempt. Invitations never become bookings, capacity, readiness or manifest state by themselves.",
  ...
  "activity_events.csv":
    "The staff activity trail: who did what, to which trip or booking or about which diver, and when. Append-only and never edited, so reading it in order reconstructs how a departure got to the state the other files describe. Each row names the kind of event in `code` and the names its sentence uses in `params`; the words themselves are DiveDay's and are written fresh in whichever language the reader has chosen, so they are not carried here.",
  ...
  "recap_pulses.csv":
    "The private word a diver sent this shop about their day, from the recap link, in the five categories the form offers (gear, briefing, boat, timing, other) plus optional words of their own. Never published anywhere and never shown to another diver — this is the feedback the shop reads on its Reviews page and acts on, which is why a shop handed its own data back gets it. categories is a JSON array of codes rather than words, for the reason observed_species_slug is a slug: the words are DiveDay's copy in the reader's language and a CSV has no reader to resolve them for. addressed_at and addressed_by_person_id are the shop's own record of having dealt with it; a row with deleted_at is one the diver took back, kept because the shop may have already read and acted on it. Not to be confused with the line a diver writes on their own postcard, which never leaves their browser and is in no file here.",
