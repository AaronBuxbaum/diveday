...
export const EXPORT_FILE_NOTES = {
  ...
  "roll_call_crew_events.csv":
    "The per-person half of each crew head count: one staff member recorded one assigned crew member aboard, not aboard, or cleared, at one checkpoint. Read it exactly like roll_call_events.csv — append-only, newest row per person and checkpoint wins, a 'cleared' row erases that checkpoint's result, and a 'not_boarded' recorded at departure carries forward until an explicit 'boarded' breaks the chain. A checkpoint was not treated as closed while any assigned crew member had no result here, and the count in roll_call_crew_attestations.csv covers anyone the assignment list never named.",
  "buddy_pairs.csv":
    'Buddy teams staff paired on each departure — two rows per pair sharing a pair_id, one per seated diver, with who made the call and when. Pairs are display and attention state on the roll call ("one buddy back aboard, one not"), never a gate: they play no part in readiness, boarding, or the head count in roll_call_events.csv. Unpairing deletes the rows, so this file holds the pairs standing at export time, not a history.',
