...
export function DiverHeader({
  ...
  status?: DiverNotice;
  /**
   * The subject-access download, for the roles allowed to take one — rendered
   * beside "Edit details" rather than on a line of its own under the header.
   * The two are peers ("do something with this record"), and a lone button
   * stranded between the header and the jump nav read as an unrelated banner.
   * A `ReactNode` because the gate is the page's to decide, not this
   * component's (`canPersonExportShopData`).
   */
  downloadRecord?: ReactNode;
  /**
    ...
    <>
      ...
      <div className="mt-4">
        ...
        {/* The disclosure lives *under* the header, not in its `actions` slot.
            ...
            form. */}
        {/* One action row for the record's two record-level doors. The
            disclosure keeps its own full-width panel by going `w-full` while
            open — which drops the download button onto the line below the
            form rather than squeezing the seven fields into whatever width is
            left beside it. */}
