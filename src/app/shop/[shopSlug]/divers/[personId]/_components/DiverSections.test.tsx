...
import { DIVER_SECTIONS, DiverSection, DiverSubNav } from "./DiverSubNav";

afterEach(cleanup);

const t = staffTranslator("en-US");
const labels = DIVER_SECTIONS.map((section) => t(section.labelKey));

function renderNav() {
  return render(<DiverSubNav ariaLabel={t("divers.subNav.ariaLabel")} labels={labels} />);
}

describe("DiverSubNav", () => {
   ...
   * browsing next week's boats, so the bar — and the page — puts money first.
   */
   ...
   * them costs a deliberate scroll; the bar must never grow a one-tap jump.
   */
...
describe("DiverSection", () => {
  it("gives the bar an anchor target that clears the sticky header", () => {
