export function NoticeBanner({ notice }: { notice?: string }) {
  const noticeText =
    notice === "captured"
      ...
      : notice === "captured-no-photo"
        ? "Card captured as pending. Photo storage isn’t set up for this shop, so no photo was attached."
        : notice === "verified"
          ? "Card marked certified. It now counts toward readiness."
          : notice === "card-deleted"
            ? "Card deleted. It no longer counts toward readiness; its history is kept for records."
            : notice === "card-restored"
              ? "Card restored. It counts toward readiness again."
              : notice === "card-restore-conflict"
                ? "Couldn't restore that card — another card now uses its number. Re-add it if you still need it."
                : notice === "person-saved"
