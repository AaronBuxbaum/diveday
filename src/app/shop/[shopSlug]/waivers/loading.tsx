...
export default function WaiverTemplateLoading() {
  return (
    <div className="animate-pulse">
      ...
      {/* Heading, description and version line all sit inside the card now, so
          the skeleton wears the card first and the bars go in it. */}
      <div className={sectionCardClass({ padding: "lg", className: "mt-10" })}>
        <div className="h-6 w-56 rounded bg-surface-sunken" />
        <div className="mt-2 h-4 w-72 max-w-full rounded bg-surface-sunken" />
        <div className="mt-4 h-96 rounded bg-surface-sunken" />
      </div>
    </div>
  );
}
