...
export function LastMinuteDealSection({
  ...
  return (
    <section id="last-minute-deal" className="mt-10 scroll-mt-6">
      <h2 className="text-lg font-semibold">Last-minute deal</h2>
      <p className="mt-1 text-sm text-muted">
        Every empty seat on a boat that's already sailing is money lost. Push a time-boxed discount
        to divers who said they're around — it expires at departure or once seats run out, whichever
        comes first.
      </p>
      ...
      {canSend ? (
        ...
        >
          ...
          >
            Send to {eligibleCount} {eligibleCount === 1 ? "diver" : "divers"}
          </SubmitButton>
        </form>
      ) : (
        <p className="mt-4 rounded-lg border border-border bg-surface px-4 py-6 text-center text-sm text-muted">
          {cancelled
            ...
            : openSeats <= 0
              ? "This trip is full — there's nothing to fill."
              : "No one on the last-minute list is around for this date yet."}
        </p>
      )}
