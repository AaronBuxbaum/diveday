...
const BOX_START = NAVIGATOR.indexOf("<input");
...
describe("the reports month navigator", () => {
  it("draws the month box at md, and Go without sm", () => {
    ...
    expect(MONTH_BOX).toContain('className={controlClassFor("md")}');
    expect(NAVIGATOR).not.toContain('size: "sm"');
  });

  it("sets the month box's width on a wrapper, not beside the control's own w-full", () => {
    expect(MONTH_BOX, "no width utility on the input itself").not.toMatch(/\bw-\d/);
    expect(NAVIGATOR).toMatch(/<div className="w-44">\s*<input/);
  });
});
