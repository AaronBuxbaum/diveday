...
export default async function NitroxPage({
  ...
  async function addCardAction(formData: FormData) {
    ...
    if (!parsed.success) redirect(`/shop/${staff.user.shopSlug}/nitrox?notice=invalid`);
    ...
    redirect(`/shop/${staff.user.shopSlug}/nitrox?notice=${card ? "captured" : "invalid"}`);
  }
  ...
  async function reviewAction(formData: FormData) {
    ...
    const updated = certificationId
      ...
      : null;
    redirect(`/shop/${staff.user.shopSlug}/nitrox?notice=${updated ? status : "invalid"}`);
  }
