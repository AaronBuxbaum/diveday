...
/**
 * The sky over a night departure, on the surface a diver reads the day they
 * sail. It informs and gates nothing; it exists because a new moon and a full
 * moon are two different night dives to pack for.
 */
describe("PackingSection — the sky over a night departure", () => {
  const keyLargo = { ...shop, latitude: 25.0865, longitude: -80.4473 } as unknown as Shop;

  function renderEvening(startsAt: Date, withShop: Shop = keyLargo) {
    const trip = {
      startsAt,
      endsAt: new Date(startsAt.getTime() + 3.5 * 60 * 60 * 1000),
      plannedDives: 2,
      waterTemperatureC: null,
      course: null,
      diveMode: "boat",
  ...
  it("says when the light goes and what moon there is", () => {
    // 7:30 PM Eastern on 2026-01-02, the demo shop's own night charter hour.
    renderEvening(new Date("2026-01-03T00:30:00Z"));
  ...
  it("says nothing at all for a departure that leaves in daylight", () => {
    // 9:00 AM Eastern: the two-tank reef trip, which needs no torch.
    renderEvening(new Date("2026-01-02T14:00:00Z"));
    expect(screen.queryByText(/Sunset/)).not.toBeInTheDocument();
  });

  it("says nothing when the shop has never set its address", () => {
    renderEvening(new Date("2026-01-03T00:30:00Z"), shop);
    expect(screen.queryByText(/Sunset/)).not.toBeInTheDocument();
  });
});
