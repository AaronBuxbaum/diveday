 ...
 * or `/reset-password/[token]`'s actual happy path through its real page and
 * server action, since the token is otherwise only ever readable from inside
 * a real email (no staff-facing display to read it from, unlike a waiver
 * link) and is hashed at rest (unlike the stateless `recap-links.ts` token
 * `e2e/visual.spec.ts` computes directly). Exists only for e2e coverage
 * (security review finding on 20260725-account-lifecycle-emails); gated
