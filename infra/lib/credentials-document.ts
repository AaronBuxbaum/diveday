...
export function renderCredentialsDocument(options: CredentialsDocumentOptions): string {
  const header = [
    ...
    "# Copy the whole thing into .env.local, or paste it into Vercel's Import .env box.",
    ...
    "#",
    "# Rotate every key (bump the number; it may only ever increase):",
    "#   pnpm infra:deploy --context credentialSerial=2",
    "#",
    "# Rotate one:",
    "#   pnpm infra:deploy --context credentialSerial:diveday-ses-sender=2",
    RULE,
    "",
  ];
