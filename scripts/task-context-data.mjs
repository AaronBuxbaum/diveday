...
export const areas = {
  ...
  notifications: {
    goal: "Extend email notification delivery (Resend live, SES dormant behind EMAIL_PROVIDER) without breaking idempotency, per-recipient locale, or delivery-status accuracy.",
    docs: [
      "docs/product/features/roadmap.md",
      "docs/product/glossary.md",
      ...
      "docs/architecture/decisions/20260802-ses-email-transition-prep.md",
      "docs/architecture/decisions/20260802-ses-adapter-and-webhook.md",
    ],
    code: [
    ...
    invariants: [
      ...
      "Delivery-attempt history is append-only; a provider-reported event applies only when newer than the stored outcome, and an unknown message id is reported without failing so Resend/SNS stops retrying it.",
      ...
      "SES stays dormant unless EMAIL_PROVIDER=ses is explicit; its SNS webhook verifies signatures and pins the expected topic ARN before trusting any event.",
    ],
    validate: [
      "pnpm test src/lib/notifications/index.test.ts src/lib/notifications/email.test.ts src/lib/notifications/events.test.ts src/lib/notifications/webhook.test.ts src/lib/notifications/sns.test.ts src/lib/notifications/ses-events.test.ts src/db/notifications.test.ts --reporter=dot",
      "pnpm check",
    ],
  },
  reviews: {
