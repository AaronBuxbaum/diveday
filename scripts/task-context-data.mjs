...
export const areas = {
  ...
  manifests: {
    ...
    code: [
      ...
      "src/db/buddy-pairs.ts",
      "src/i18n/buddy-labels.ts",
    ...
    tests: [
      ...
      "src/db/buddy-pairs.test.ts",
      "e2e/manifest.spec.ts",
      "e2e/buddy-pairs.spec.ts",
    ],
    invariants: [
      ...
      "A buddy team has two or more members; a member is a booking or a crew person; a diver is on at most one team per departure and crew may be on several.",
      "Buddy teams inform and never act \u2014 no readiness, admission, capacity, or checkpoint-completeness input, and no message to anyone.",
      "Every act on a team (form/add/remove/dissolve) appends to buddy_team_events with the member names as they stood, which outlives the membership rows.",
    ],
    validate: [
  ...
  bookings: {
    ...
    code: [
      ...
      "src/app/actions/seat-diver.ts",
      "src/app/actions/seat-diver-surfaces.ts",
      "src/lib/trips.ts",
      "src/app/s/[shopSlug]",
      "src/app/shop/[shopSlug]/bookings/new",
      "src/app/shop/[shopSlug]/schedule/board",
      "src/app/shop/[shopSlug]/trips",
    ],
    tests: ["src/db/bookings.test.ts", "src/db/seat-diver.test.ts", "e2e"],
    invariants: [
      ...
      "Every staff door that seats a diver goes through seatDiver, so the waiver, the activity trail, and the analytics event can never be skipped by one surface.",
    ],
    validate: [
  ...
  notifications: {
    ...
    invariants: [
      ...
      "A reserved test/demo recipient domain never reaches SES.",
      "Delivery fails closed: a provider rejection surfaces as failure without exposing provider details, and a throttling/5xx response is retried by the AWS SDK's own backoff.",
      ...
      "Delivery-attempt history is append-only; a provider-reported event applies only when newer than the stored outcome, and an unknown message id is reported without failing so the webhook's caller stops retrying it.",
      "SES's SNS delivery webhook verifies the message signature and TopicArn by hand before any event is trusted; a stale, tampered, mismatched-topic, or malformed payload is rejected.",
