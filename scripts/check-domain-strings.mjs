 ...
 * scanned roots.
 *
 * The scanner half is exported and the runner is behind a `main()` guard, the shape
 * `check-e2e-hygiene.mjs` uses and `check-copy.mjs` took in #1130 — so
 * `scripts/check-domain-strings.test.mjs` can pin every rule in both directions without the
 * module calling `process.exit` at import (issue #1258). Which direction matters is worth
...
export function findDomainStrings(source) {
...
export function findProseLiterals(source) {
  const stripped = stripComments(source);
  const rawLines = source.split("\n");
